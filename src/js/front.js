    ...
    class SmoothScroll extends Component {
        ...
        _init() {
            ...
            if (typeof wecodeartSupportModulesSmoothscroll !== 'undefined') {
                this._config = { ...this._config, ...wecodeartSupportModulesSmoothscroll };
            }
        ...
        _handleKeydown(event) {
            try {
                ...
                if (event.defaultPrevented ||
                    ...
                    this._isNodeName(target, 'input') && !buttonTypes.test(target.type) ||
                    this._isNodeName(this._activeElement, 'video') ||
        ...
        _shouldHandleWheel(event) {
            try {
                ...
                if (this._isNodeName(this._activeElement, 'embed') ||
                    (this._isNodeName(target, 'embed') && /\.pdf/i.test(target.src)) ||
                    this._isNodeName(this._activeElement, 'object')) {
        ...
        _findOverflowingElement(target) {
            ...
            while (target) {
                ...
                if (rootScrollHeight === target.scrollHeight) {
                    ...
                    if (this._isFrame && this._isContentOverflowing(this._root) ||
                        !this._isFrame && isOverflowCSS) {
        ...
        _allDeltasDivisableBy(divisor) {
            return (this._isDivisible(this._deltaBuffer[0], divisor) &&
                this._isDivisible(this._deltaBuffer[1], divisor) &&
                this._isDivisible(this._deltaBuffer[2], divisor));
        }
